...
- **Cron / Scheduled Runs** — `lib/cron-manager.ts` uses `node-cron` (Asia/Riyadh timezone). DB table `bot_schedules` (Drizzle/PostgreSQL) stores per-bot schedules with expression, label, enabled flag, last-run timestamp + status. On server boot, `initCronManager()` re-registers all enabled schedules. Endpoints (Clerk auth + ownership): `GET/POST /api/bots/:id/schedules`, `PATCH/DELETE /api/bots/:id/schedules/:sid`. On trigger: starts bot if stopped, marks `skipped` if already running. Frontend: "الجدولة" tab in `/agent` right panel — preset buttons (كل ساعة / يومياً / أسبوعياً / شهرياً), custom expression input, enable/disable toggle, last-run badge (✓/تخطّى/خطأ), delete.
